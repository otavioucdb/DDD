...
public class TodosHospedesImpl implements TodosHospedes {
	...
	public List<Hospede> getPorParteNome(String parteNome) {
		return provider.get().createQuery(
				"from Hospede where upper(nome) like :parteNome").setParameter(
				"parteNome", "%" + parteNome.toUpperCase() + "%")
				.getResultList();
	}

	@SuppressWarnings("unchecked")
	public Hospede getPorNomeCompleto(String nome) {
		return (Hospede) provider.get().createQuery(
				"from Hospede where upper(nome) = :nome").setParameter("nome",
				nome.toUpperCase()).getSingleResult();
	}
